...
// const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxy0BoUP3CEwVgPxT6bFxDd2POdWMJfNcI2mq1OkBkWLX1UKF4VVrRwOco1dtVLR1gW/exec";
...
const APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbxy0BoUP3CEwVgPxT6bFxDd2POdWMJfNcI2mq1OkBkWLX1UKF4VVrRwOco1dtVLR1gW/exec";
...
export default function VerifyIdentityForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      /*
       * KEY FIX: Send as URL-encoded string with no-cors mode.
       * - "no-cors" avoids the CORS preflight that causes Google to redirect
       *   the request to an echo/cache URL, stripping the POST body.
       * - URL-encoded body is readable via e.parameter in Apps Script.
       * - Because no-cors gives an opaque response, we can't read the JSON
       *   reply — but the script WILL execute and send the emails correctly.
       *   We optimistically show success after the fetch completes.
       */
      ...
      await fetch(APPS_SCRIPT_URL, {
        method: "POST",
        mode: "no-cors",                               // ← the critical fix
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: encoded,
      });

      // no-cors returns opaque response (status 0) — treat completion as success
      setStatus("success");
    } catch (err) {
      setStatus("error");
    }
